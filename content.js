(() => {
  ...
  const storageGet = (area, keys) => new Promise((resolve) => area.get(keys, (value) => resolve(chrome.runtime.lastError ? {} : value || {})));
  const storageSet = (area, data) => new Promise((resolve) => area.set(data, () => resolve(!chrome.runtime.lastError)));
  const storageRemove = (area, keys) => new Promise((resolve) => !keys.length ? resolve(true) : area.remove(keys, () => resolve(!chrome.runtime.lastError)));
  ...
  async function syncDrawingMaps(fibDrawings, toolDrawings) {
    const current = await storageGet(chrome.storage.sync, null), desired = {};
    ...
    await storageRemove(chrome.storage.sync, stale); if (Object.keys(desired).length) await storageSet(chrome.storage.sync, desired);
    await storageRemove(chrome.storage.sync, ["fibDrawings", "toolDrawings"]);
  }
  async function storeGet(keys) {
    const [local, allSynced] = await Promise.all([storageGet(chrome.storage.local, keys), storageGet(chrome.storage.sync, null)]), synced = {};
    ...
    if (Object.keys(settings).length) storageSet(chrome.storage.sync, settings);
    storageSet(chrome.storage.local, merged);
    if ("fibDrawings" in allSynced || "toolDrawings" in allSynced || (!Object.keys(shards.fibDrawings).length && !Object.keys(shards.toolDrawings).length)) syncWriteQueue = syncWriteQueue.then(() => syncDrawingMaps(merged.fibDrawings, merged.toolDrawings));
    return merged;
  }
  ...
  function rebuild() {
    ...
    if (tf().biweekly) {
      ...
      for (const row of rows) {
        ...
        if (!candle) buckets.set(key, { ...row });
        else { candle.high = Math.max(candle.high, row.high); candle.low = Math.min(candle.low, row.low); candle.close = row.close; }
      }
      rows = [...buckets.values()];
    }
  ...
  function drawToolDrawing(ctx, drawing, start, x, y, w, h, low, high, selected = false, preview = false) {
    ...
    ctx.setLineDash([]);
    if (drawing.type === "trend") { const style = drawing.style || {}, dash = style.dash || "solid"; ctx.strokeStyle = style.color || "#f23645"; ctx.lineWidth = Math.max(1, Math.min(8, Number(style.lineWidth) || 4)); ctx.setLineDash(dash === "dash" ? [10, 7] : dash === "dot" ? [2, 5] : []); ctx.beginPath(); ctx.moveTo(ax, ay); ctx.lineTo(bx, by); ctx.stroke(); ctx.setLineDash([]); }
    ...
    const extraBottom = drawing.type === "long" && Number.isFinite(longStopY) ? longStopY : bottom; return { left: left - 10, right: Math.max(right, left + 24) + 10, top: Math.min(top, extraBottom) - 28, bottom: Math.max(bottom, extraBottom) + 28, anchors, ...(drawing.type === "trend" ? { line: true, ax, ay, bx, by } : {}) };
  }
  function renderPrice(start, end) {
    ...
    let rawLow = Math.min(...rows.map((r) => r.low)), rawHigh = Math.max(...rows.map((r) => r.high)); const baseRange = Math.max(rawHigh - rawLow, rawHigh * .001) * 1.12, baseMid = (rawHigh + rawLow) / 2, range = baseRange / Math.max(.15, state.priceScale), mid = baseMid + state.priceShift * baseRange, low = mid - range / 2, high = mid + range / 2; state.lastPriceView = { low, high, range, baseRange, baseMid };
    ...
    const liveIndex = state.rows.length - 1;
    if (liveIndex >= start && liveIndex < end) { const livePrice = state.rows[liveIndex].close, liveY = py(livePrice), liveLabel = livePrice.toLocaleString(undefined, { maximumFractionDigits: 6 }); ctx.setLineDash([2, 3]); ctx.strokeStyle = "rgba(17,24,39,.78)"; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(x, liveY); ctx.lineTo(x + w, liveY); ctx.stroke(); ctx.setLineDash([]); ctx.font = "bold 10px Arial"; ctx.fillStyle = "#171b26"; ctx.fillRect(x + w, liveY - 9, 58, 18); ctx.fillStyle = "#fff"; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.fillText(liveLabel, x + w + 29, liveY); }
    ...
    if (liveIndex >= start && liveIndex < end) { const livePrice = state.rows[liveIndex].close; priceScaleLabel(ctx, plotRight, py(livePrice), livePrice, "#171b26"); }
  ...
  function connect() {
    ...
    socket.onmessage = (event) => {
      ...
      const next = { time: Number(k.t), open: Number(k.o), high: Number(k.h), low: Number(k.l), close: Number(k.c) };
    ...
    socket.onclose = () => { if (state.visible && state.socket === socket) setTimeout(connect, 2500); };
  }
  function select(label) {
    ...
    shadow.querySelectorAll(".tab").forEach((b) => b.classList.toggle("active", b.dataset.tf === label)); storeSet({ selected: label }); load();
  }
  function geometry() {
  ...
  function enableTwoPointTool(canvas, button, type, pane = "price", bindButton = true) {
    ...
    canvas.addEventListener("pointerdown", (e) => { if (state.drawingTool !== type || (state.toolDraft && state.toolDraft.pane !== pane)) return; e.preventDefault(); e.stopImmediatePropagation(); const point = snapped(pointFromEvent(e), e.shiftKey); if (state.toolDraft) { state.toolDraft.b = point; finish(); return; } state.toolDraft = { type, pane, a: point, b: point }; dragStart = point; canvas.setPointerCapture(e.pointerId); render(); }, true);
    canvas.addEventListener("pointermove", (e) => { if (state.drawingTool !== type || !state.toolDraft || state.toolDraft.pane !== pane) return; state.toolDraft.b = snapped(pointFromEvent(e), e.shiftKey); render(); });
  ...
  function toggle(force) {
    ...
    if (state.visible) refreshSymbol(true); else if (state.socket) { state.socket.onclose = null; state.socket.close(); state.socket = null; }
  }
  function refreshSymbol(force = false) {
    const next = symbolFromUrl(); if (!next) { shadow.querySelector(".symbol").textContent = "Mở trang Spot /trade/…"; return; }
    if (!force && next === state.symbol) return; state.symbol = next; shadow.querySelector(".symbol").textContent = next; load();
  }
  function mount() {
    ...
    const refreshButton = shadow.querySelector(".refresh"); refreshButton.innerHTML = ICONS.refresh; refreshButton.onclick = () => { state.panBars = 0; load(); };
  ...
  async function init() {
    ...
    chrome.runtime.onMessage.addListener((m) => { if (m?.type === "TOGGLE_RSI_MTF") toggle(); });
    chrome.storage.onChanged.addListener((changes, area) => {
      ...
      if (Object.keys(changes).some((key) => key.startsWith(DRAWING_SYNC_PREFIX))) {
        clearTimeout(syncReloadTimer); syncReloadTimer = setTimeout(async () => {
          const shards = drawingsFromShards(await storageGet(chrome.storage.sync, null));
          state.fibDrawings = shards.fibDrawings; state.toolDrawings = shards.toolDrawings; state.selectedDrawing = null;
          await storageSet(chrome.storage.local, { fibDrawings: state.fibDrawings, toolDrawings: state.toolDrawings });
