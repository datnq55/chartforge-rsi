(() => {
  ...
  let shadow, resizeTimer, syncReloadTimer, replayTimer, replayPanFrame, countdownTimer, activeDrawingEdit = null, drawingShiftPressed = false, syncWriteQueue = Promise.resolve();
  ...
  function renderPrice(start, end) {
    ...
    const legend = priceLegendData();
    if (legend) {
      ctx.font = "12px Arial"; ctx.textAlign = "left"; ctx.textBaseline = "middle"; let tx = x + 7, ty = y + 11;
      ctx.fillStyle = legend.candleColor; ctx.beginPath(); ctx.arc(tx, ty, 4, 0, Math.PI * 2); ctx.fill(); tx += 11;
      for (const part of legend.parts) { ctx.fillStyle = "#111"; ctx.fillText(part, tx, ty); tx += ctx.measureText(part).width + 9; }
      ctx.fillStyle = legend.changeColor; ctx.fillText(legend.delta, tx, ty);
    }
  }
  function render() {
  ...
  function mount() {
    ...
    resetButton.onclick = () => { state.panBars = 0; state.zoomBars = RESET_ZOOM_BARS; state.priceShift = 0; state.priceScale = 1; state.pricePercent = 64; pricePane.style.flexBasis = "64%"; zoom.value = RESET_ZOOM_BARS; output.value = RESET_ZOOM_BARS; render(); storeSet({ panBars: 0, zoomBars: RESET_ZOOM_BARS, priceShift: 0, priceScale: 1, pricePercent: 64 }); };
