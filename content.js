(() => {
  ...
  const TIME_SCALE_DRAG_PX_PER_BAR = 5;
  ...
  let shadow, resizeTimer, syncReloadTimer, replayTimer, replayPanFrame, countdownTimer, syncWriteQueue = Promise.resolve();
  const svg = (body) => `<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">${body}</svg>`;
  const runtimeUrl = (path) => { try { return chrome.runtime.getURL(path); } catch { return ""; } };
  const collapseContent = (collapsed) => collapsed ? `<img src="${runtimeUrl("icons/icon32.png")}" alt="" draggable="false">` : ICONS.collapse;
  const ICONS = {
  ...
  const storageArea = (name) => { try { return chrome?.storage?.[name] || null; } catch { return null; } };
    ...
    return new Promise((resolve) => {
      ...
      try {
        ...
        area[method](...args, (value) => {
          let failed = false;
          try { failed = Boolean(chrome?.runtime?.lastError); } catch { failed = true; }
  ...
  function updateCandleCountdown(now = Date.now()) {
    ...
    const hidden = !state.visible || state.collapsed || state.replay.open; countdown.classList.toggle("hidden", hidden); if (hidden) return;
    const boundary = state.countdownTimeframe === tf().label ? candleCloseBoundary(state.rows.at(-1)) : null;
    countdown.textContent = Number.isFinite(boundary) ? `Đóng nến sau ${formatCandleCountdown(boundary - now)}` : "Đóng nến sau --:--:--";
  }
  ...
  function timeTicks(start, width) {
    const endSlot = start + state.zoomBars - 1, label = tf().label, months = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"], ticks = [];
    if (["D", "3D", "1W", "2W", "M"].includes(label)) {
      const first = new Date(timeAt(start)), last = new Date(timeAt(endSlot)), monthSpan = Math.max(1, (last.getUTCFullYear() - first.getUTCFullYear()) * 12 + last.getUTCMonth() - first.getUTCMonth()), target = Math.max(3, Math.floor(width / 105));
      let monthStep;
      if (label === "M") { const roughYears = monthSpan / 12 / target, yearStep = [1,2,5,10,20,50].find((n) => n >= roughYears) || 100; monthStep = yearStep * 12; }
      else if (label === "1W" || label === "2W") { const rough = Math.max(6, monthSpan / target); monthStep = [6,12,24,36,60,120].find((n) => n >= rough) || 120; }
      else { const rough = Math.max(1, monthSpan / target); monthStep = [1,2,3,6,12,24,36,60].find((n) => n >= rough) || 60; }
      const before = new Date(timeAt(start - 1)); let previousMonthId = before.getUTCFullYear() * 12 + before.getUTCMonth();
      for (let i = start; i <= endSlot; i++) {
        const d = new Date(timeAt(i)), monthId = d.getUTCFullYear() * 12 + d.getUTCMonth();
        if (previousMonthId != null && monthId !== previousMonthId && monthId % monthStep === 0) { const month = d.getUTCMonth(), yearBoundary = month === 0; ticks.push({ index: i, label: yearBoundary ? String(d.getUTCFullYear()) : months[month], major: yearBoundary }); }
        previousMonthId = monthId;
      }
      return ticks;
    }
    const first = timeAt(start), last = timeAt(endSlot), spanDays = Math.max(1, (last - first) / 86400000), rough = spanDays / Math.max(3, Math.floor(width / 115)), choices = [1,2,3,5,7,10,14,21,30], stepDays = choices.find((n) => n >= rough) || 30, before = new Date(timeAt(start - 1)); let previousDay = `${before.getUTCFullYear()}-${before.getUTCMonth()}-${before.getUTCDate()}`, previousMonth = `${before.getUTCFullYear()}-${before.getUTCMonth()}`;
    for (let i = start; i <= endSlot; i++) {
      const d = new Date(timeAt(i)), monthKey = `${d.getUTCFullYear()}-${d.getUTCMonth()}`, dayKey = `${monthKey}-${d.getUTCDate()}`, monthChanged = previousMonth != null && monthKey !== previousMonth;
      if (monthChanged) ticks.push({ index: i, label: months[d.getUTCMonth()], major: true });
      else if (dayKey !== previousDay) { const ordinal = Math.floor(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate()) / 86400000); if (ordinal % stepDays === 0 || !ticks.length) ticks.push({ index: i, label: String(d.getUTCDate()), major: false }); }
      previousMonth = monthKey; previousDay = dayKey;
    }
    return ticks;
  }
  ...
  function priceGuide(ctx, fromX, plotRight, py, price, color) {
    ctx.setLineDash([4, 4]); ctx.strokeStyle = color; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(fromX, py); ctx.lineTo(plotRight, py); ctx.stroke(); ctx.setLineDash([]); priceScaleLabel(ctx, plotRight, py, price, color);
  }
  ...
  function drawReplaySelection(ctx, start, x, y, w, h) {
    ...
    ctx.fillStyle = "rgba(255,255,255,.68)"; ctx.fillRect(px, y, x + w - px, h);
    ctx.strokeStyle = "#2962ff"; ctx.lineWidth = 1.5; ctx.setLineDash([]); ctx.beginPath(); ctx.moveTo(px, y); ctx.lineTo(px, y + h); ctx.stroke();
    ctx.fillStyle = "#2962ff"; ctx.font = "16px Arial"; ctx.textAlign = "center"; ctx.textBaseline = "top"; ctx.fillText("✂", px, y + 6);
  }
  function renderPrice(start, end) {
    const canvas = shadow.querySelector(".price-canvas"); if (!canvas || !state.rows?.length) return;
    const rect = canvas.getBoundingClientRect(); if (rect.width < 40 || rect.height < 40) return;
    const ratio = Math.max(1, devicePixelRatio || 1); canvas.width = Math.round(rect.width * ratio); canvas.height = Math.round(rect.height * ratio);
    ...
    for (const tick of timeTicks(start, w)) { const px = x + (tick.index - start) / Math.max(1, state.zoomBars - 1) * w; ctx.strokeStyle = tick.major ? "rgba(0,0,0,.14)" : "rgba(0,0,0,.075)"; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(px, y); ctx.lineTo(px, y + h); ctx.stroke(); }
    for (let g = 0; g <= 4; g++) { const gy = y + g / 4 * h, price = high - g / 4 * (high - low); ctx.strokeStyle = "rgba(0,0,0,.09)"; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(x, gy); ctx.lineTo(x + w, gy); ctx.stroke(); ctx.fillStyle = "#111"; ctx.font = "10px Arial"; ctx.textAlign = "left"; ctx.fillText(price.toLocaleString(undefined, { maximumFractionDigits: 6 }), x + w + 5, gy + 3); }
    const slot = w / Math.max(1, state.zoomBars - 1), bodyWidth = Math.max(2, Math.min(12, slot * .68));
    rows.forEach((row, j) => { const px = x + (dataStart + j - start) / Math.max(1, state.zoomBars - 1) * w, up = row.close >= row.open; ctx.strokeStyle = "#000"; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(px, py(row.high)); ctx.lineTo(px, py(row.low)); ctx.stroke(); const top = py(Math.max(row.open, row.close)), bottom = py(Math.min(row.open, row.close)); ctx.fillStyle = up ? "#fff" : "#000"; ctx.fillRect(px - bodyWidth / 2, top, bodyWidth, Math.max(1, bottom - top)); ctx.strokeStyle = "#000"; ctx.strokeRect(px - bodyWidth / 2, top, bodyWidth, Math.max(1, bottom - top)); });
    const liveIndex = state.rows.length - 1, liveColor = state.replay.active ? "#2962ff" : "#171b26";
    if (liveIndex >= start && liveIndex < end) { const livePrice = state.rows[liveIndex].close, liveY = py(livePrice), liveLabel = livePrice.toLocaleString(undefined, { maximumFractionDigits: 6 }); ctx.setLineDash([2, 3]); ctx.strokeStyle = liveColor; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(x, liveY); ctx.lineTo(x + w, liveY); ctx.stroke(); ctx.setLineDash([]); ctx.font = "bold 10px Arial"; ctx.fillStyle = liveColor; ctx.fillRect(x + w, liveY - 9, 58, 18); ctx.fillStyle = "#fff"; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.fillText(liveLabel, x + w + 29, liveY); }
    ...
    ctx.strokeStyle = "rgba(0,0,0,.12)"; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(plotRight + .5, 0); ctx.lineTo(plotRight + .5, rect.height); ctx.stroke();
    ...
    if (state.hoverIndex != null && state.hoverIndex >= start && state.hoverIndex < start + state.zoomBars) { const px = x + (state.hoverIndex - start) / Math.max(1, state.zoomBars - 1) * w; ctx.setLineDash([6,5]); ctx.strokeStyle = "rgba(55,65,81,.75)"; ctx.beginPath(); ctx.moveTo(px, y); ctx.lineTo(px, y + h); ctx.stroke(); ctx.setLineDash([]); }
    if (state.hoverPane === "price" && state.hoverYRatio != null) { const hy = y + state.hoverYRatio * h, price = high - state.hoverYRatio * (high - low); ctx.setLineDash([3,3]); ctx.strokeStyle = "#111"; ctx.beginPath(); ctx.moveTo(x, hy); ctx.lineTo(plotRight, hy); ctx.stroke(); ctx.setLineDash([]); priceScaleLabel(ctx, plotRight, hy, price, "#171b26"); }
    const hoverRow = state.hoverPane === "price" ? state.rows[state.hoverIndex] : null;
    if (hoverRow) {
      const change = hoverRow.close - hoverRow.open, percent = hoverRow.open ? change / hoverRow.open * 100 : 0, value = (n) => n.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 6 });
      ctx.font = "12px Arial"; ctx.textAlign = "left"; ctx.textBaseline = "middle"; let tx = x + 7, ty = y + 11;
      ctx.fillStyle = hoverRow.close >= hoverRow.open ? "#089981" : "#f23645"; ctx.beginPath(); ctx.arc(tx, ty, 4, 0, Math.PI * 2); ctx.fill(); tx += 11;
      for (const part of [`O${value(hoverRow.open)}`, `H${value(hoverRow.high)}`, `L${value(hoverRow.low)}`, `C${value(hoverRow.close)}`]) { ctx.fillStyle = "#111"; ctx.fillText(part, tx, ty); tx += ctx.measureText(part).width + 9; }
      const delta = `${change >= 0 ? "+" : ""}${value(change)} (${percent >= 0 ? "+" : ""}${percent.toFixed(2)}%)`; ctx.fillStyle = change >= 0 ? "#089981" : "#f23645"; ctx.fillText(delta, tx, ty);
    }
  }
  function render() {
    ...
    for (let i = start; i < end; i++) {
      if (main[i] > 80 || main[i] < 20) {
        ...
        ctx.fillRect(x + (i - start) * barWidth, y, Math.ceil(barWidth) + 1, h);
      }
    }
    for (const level of [10,20,30,40,50,60,70,80,90]) {
      const py = y + h * (1 - level / 100), major = level === 30 || level === 70;
      ctx.strokeStyle = major ? "rgba(255,255,255,.82)" : "rgba(255,255,255,.38)"; ctx.lineWidth = major ? 1.6 : .7; ctx.setLineDash(major ? [] : [4,4]);
    ...
    drawReplaySelection(ctx, start, x, y, w, h);
    ctx.strokeStyle = "rgba(255,255,255,.55)"; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(x, y + h); ctx.lineTo(x + w, y + h); ctx.stroke();
    ctx.font = "10px Arial"; ctx.fillStyle = "#111"; ctx.textAlign = "center"; ctx.textBaseline = "top";
    const ticks = timeTicks(start, w);
    for (let t = 0; t < ticks.length; t++) {
      const tick = ticks[t], px = x + (tick.index - start) / Math.max(1, state.zoomBars - 1) * w;
      ctx.strokeStyle = tick.major ? "rgba(0,0,0,.14)" : "rgba(0,0,0,.075)"; ctx.beginPath(); ctx.moveTo(px, y); ctx.lineTo(px, y + h); ctx.stroke();
      ctx.fillStyle = "#111"; ctx.textAlign = t === 0 ? "left" : t === ticks.length - 1 ? "right" : "center"; ctx.font = tick.major ? "bold 10px Arial" : "10px Arial"; ctx.fillText(tick.label, px, y + h + 6);
    }
    ...
    if (state.hoverIndex != null && state.hoverIndex >= start && state.hoverIndex < start + state.zoomBars) {
      const px = x + (state.hoverIndex - start) / Math.max(1, state.zoomBars - 1) * w;
      ctx.setLineDash([6, 5]); ctx.strokeStyle = "rgba(55,65,81,.75)"; ctx.lineWidth = 1;
  ...
  function requestExitReplay() {
    if (!state.replay.open) return; setReplayPlaying(false); stopReplayPanAnimation(); shadow?.querySelector(".replay-menu")?.classList.remove("show"); shadow?.querySelector(".replay-date-dialog")?.classList.remove("show"); shadow?.querySelector(".replay-exit-dialog")?.classList.add("show");
  }
  ...
  function exitReplay() {
    ...
    shadow?.querySelector(".replay-menu")?.classList.remove("show"); shadow?.querySelector(".replay-date-dialog")?.classList.remove("show"); shadow?.querySelector(".replay-exit-dialog")?.classList.remove("show"); updateReplayUI(); ignoreFailure(load());
  }
  ...
  function enableDrawingSelection(canvas, pane = "price") {
    ...
    canvas.addEventListener("pointerdown", (e) => {
      ...
      if (state.drawingTool) return; const rect = canvas.getBoundingClientRect(), px = e.clientX - rect.left, py = e.clientY - rect.top, hitTest = (area) => { if (area.pane !== pane) return false; if (!area.line) return px >= area.left && px <= area.right && py >= area.top && py <= area.bottom; const dx = area.bx - area.ax, dy = area.by - area.ay, length2 = dx * dx + dy * dy, t = length2 ? Math.max(0, Math.min(1, ((px - area.ax) * dx + (py - area.ay) * dy) / length2)) : 0, lx = area.ax + t * dx, ly = area.ay + t * dy; return Math.hypot(px - lx, py - ly) <= 9; }, hit = [...state.drawingHitAreas].reverse().find(hitTest);
      ...
      const anchor = hit.anchors?.find((a) => Math.hypot(px - a.x, py - a.y) <= 10), original = structuredClone(getDrawing(nextSelection)); edit = { selection: nextSelection, original, anchor: anchor?.name || null, startPoint: pointFromEvent(e), moved: false }; canvas.setPointerCapture(e.pointerId); render(); requestAnimationFrame(updateDrawingMenu);
    }, true);
    canvas.addEventListener("pointermove", (e) => { if (!edit) return; const current = pointFromEvent(e), drawing = structuredClone(edit.original); updateDrawingPointerHover(canvas, pane, e, current.index); if (!edit.moved) pushDrawingHistory(); edit.moved = true; if (edit.anchor) { let price = current.price; const epsilon = Math.max(Math.abs(drawing.a.price) * .0001, 1e-8); if (drawing.type === "long" && edit.anchor === "b") price = Math.max(drawing.a.price + epsilon, price); if (drawing.type === "long" && edit.anchor === "c") price = Math.min(drawing.a.price - epsilon, price); drawing[edit.anchor] = { ...(drawing[edit.anchor] || drawing.a), time: edit.anchor === "c" ? drawing.a.time : current.time, price }; } else { const deltaBars = nearestIndex(current.time) - nearestIndex(edit.startPoint.time), deltaPrice = current.price - edit.startPoint.price; for (const name of ["a", "b", "c"]) if (drawing[name]) drawing[name] = { ...drawing[name], time: timeAt(nearestIndex(drawing[name].time) + deltaBars), price: drawing[name].price + deltaPrice }; } setDrawing(edit.selection, drawing); render(); });
    const stopEdit = () => { if (!edit) return; if (edit.moved) storeSet(edit.selection.type === "fib" ? { fibDrawings: state.fibDrawings } : { toolDrawings: state.toolDrawings }); edit = null; updateDrawingMenu(); };
    canvas.addEventListener("pointerup", stopEdit); canvas.addEventListener("pointercancel", stopEdit);
  }
  ...
  function enableTwoPointTool(canvas, button, type, pane = "price", bindButton = true) {
    ...
    const snapped = (point, shift) => { if (type !== "trend" || !shift || !state.toolDraft) return point; const a = state.toolDraft.a, dx = Math.abs(point.clientX - a.clientX), dy = Math.abs(point.clientY - a.clientY); return dx >= dy ? { ...point, price: a.price, clientY: a.clientY } : { ...point, time: a.time, clientX: a.clientX }; };
    const finish = () => { if (!state.toolDraft) return; pushDrawingHistory(); const key = fibKey(), a = state.toolDraft.a, b = state.toolDraft.b, targetPrice = type === "long" && b.price < a.price ? a.price + Math.abs(b.price - a.price) : b.price, drawing = { type, pane, a: { time: a.time, price: a.price }, b: { time: b.time, price: targetPrice }, ...(type === "trend" ? { style: { ...state.toolDefaults.trend } } : {}), ...(type === "long" ? { c: { time: a.time, price: a.price - Math.max(Math.abs(targetPrice - a.price), Math.abs(a.price) * .001) } } : {}) }, list = [...toolDrawings(), drawing]; state.toolDrawings = { ...state.toolDrawings, [key]: list }; state.selectedDrawing = { type, key, index: list.length - 1, pane }; state.menuPosition = null; state.toolDraft = null; state.drawingTool = null; button.classList.remove("active"); storeSet({ toolDrawings: state.toolDrawings }); render(); requestAnimationFrame(updateDrawingMenu); };
    ...
    canvas.addEventListener("pointerdown", (e) => { if (state.drawingTool !== type || (state.toolDraft && state.toolDraft.pane !== pane)) return; e.preventDefault(); e.stopImmediatePropagation(); const rawPoint = pointFromEvent(e), point = snapped(rawPoint, e.shiftKey); updateDrawingPointerHover(canvas, pane, e, rawPoint.index); if (state.toolDraft) { state.toolDraft.b = point; finish(); return; } state.toolDraft = { type, pane, a: point, b: point }; dragStart = point; canvas.setPointerCapture(e.pointerId); render(); }, true);
    canvas.addEventListener("pointermove", (e) => { if (state.drawingTool !== type || !state.toolDraft || state.toolDraft.pane !== pane) return; const rawPoint = pointFromEvent(e); state.toolDraft.b = snapped(rawPoint, e.shiftKey); updateDrawingPointerHover(canvas, pane, e, rawPoint.index); render(); });
  ...
  function enablePan(canvas) {
    ...
    canvas.addEventListener("pointermove", (e) => {
      ...
      if (mode === "timeZoom") { const nextZoom = Math.max(20, Math.min(1000, Math.round(startZoom + (e.clientX - startX) / TIME_SCALE_DRAG_PX_PER_BAR))), currentRatio = Math.max(0, Math.min(1, (e.clientX - rect.left - 8) / Math.max(1, rect.width - 82))), nextStart = Math.round(anchorIndex - currentRatio * (nextZoom - 1)); state.zoomBars = nextZoom; state.panBars = clampPan(state.closes.length - nextZoom - nextStart, nextZoom); const slider = shadow.querySelector(".zoom input"); slider.value = state.zoomBars; shadow.querySelector(".zoom output").value = state.zoomBars; }
  ...
  function enableCrosshair(canvas) {
    canvas.addEventListener("mousemove", (e) => {
      ...
      state.hoverIndex = Math.round(start + ratio * Math.max(0, state.zoomBars - 1)); render();
    });
  ...
  function enableSplitter(splitter, pricePane, rsiPane) {
    ...
    splitter.addEventListener("pointermove", (e) => {
      ...
      state.pricePercent = Math.max(20, Math.min(80, startPercent + (e.clientY - startY) / Math.max(1, total) * 100)); pricePane.style.flexBasis = `${state.pricePercent}%`; render();
    });
  ...
  function mount() {
    ...
    const topbar = document.createElement("div"); topbar.className = "topbar"; topbar.appendChild(shadow.querySelector(".chart-identity")); topbar.appendChild(shadow.querySelector(".tabs")); const topbarSeparator = document.createElement("span"); topbarSeparator.className = "topbar-separator"; topbarSeparator.setAttribute("aria-hidden", "true"); topbar.appendChild(topbarSeparator); const replayButton = document.createElement("button"); replayButton.className = "replay-button"; replayButton.title = "Bar Replay"; replayButton.innerHTML = `${ICONS.replay}<span>Replay</span>`; topbar.appendChild(replayButton); topbar.appendChild(shadow.querySelector(".status")); rsiPane.before(topbar);
    const candleCountdown = document.createElement("span"); candleCountdown.className = "candle-countdown"; candleCountdown.setAttribute("aria-live", "off"); bottomBar.prepend(candleCountdown);
    ...
    const replayExitDialog = document.createElement("div"); replayExitDialog.className = "replay-exit-dialog"; replayExitDialog.setAttribute("role", "dialog"); replayExitDialog.setAttribute("aria-modal", "true"); replayExitDialog.innerHTML = '<div class="replay-exit-title">Thoát Bar Replay?</div><div class="replay-exit-message">Chart sẽ trở về dữ liệu realtime và kết thúc phiên replay hiện tại.</div><div class="replay-date-actions"><button class="replay-exit-cancel">Ở lại</button><button class="replay-exit-confirm">Thoát replay</button></div>'; panel.appendChild(replayExitDialog);
    ...
    replayExitDialog.querySelector(".replay-exit-cancel").onclick = () => replayExitDialog.classList.remove("show"); replayExitDialog.querySelector(".replay-exit-confirm").onclick = exitReplay;
    ...
    drawingToolbar.querySelector(".clear-drawings").onclick = () => {
      const key = fibKey();
      if (![...(state.fibDrawings[key] || []), ...(state.toolDrawings[key] || [])].length) return;
      if (!confirm(`Xóa tất cả drawing của ${state.symbol || "chart"} trên mọi khung thời gian?`)) return;
      pushDrawingHistory();
    ...
    pricePane.style.flexBasis = `${state.pricePercent}%`; const zoom = shadow.querySelector(".zoom input"), output = shadow.querySelector(".zoom output"); zoom.min = 20; zoom.max = 1000; zoom.value = state.zoomBars; output.value = state.zoomBars;
    ...
    resetButton.onclick = () => { state.panBars = 0; state.zoomBars = 120; state.priceShift = 0; state.priceScale = 1; state.pricePercent = 64; pricePane.style.flexBasis = "64%"; zoom.value = 120; output.value = 120; render(); storeSet({ panBars: 0, zoomBars: 120, priceShift: 0, priceScale: 1, pricePercent: 64 }); };
    ...
    addEventListener("keydown", (e) => {
      if (e.key === "Escape") { if (replayExitDialog.classList.contains("show")) replayExitDialog.classList.remove("show"); else if (replayDateDialog.classList.contains("show")) hideReplayDateDialog(); else replayMenu.classList.remove("show"); return; }
  ...
  async function init() {
    ...
    try { chrome.runtime.onMessage.addListener((m) => { if (m?.type === "TOGGLE_RSI_MTF") toggle(); }); } catch { /* An already-open tab may retain an invalidated content-script context after extension reload. */ }
    ...
    }); } catch { /* Storage listeners are unavailable after extension context invalidation. */ }
  }
  ignoreFailure(init());
})();
