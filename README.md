...
- `web/firestore.rules` khóa dữ liệu theo UID và validate symbol/type/field. Rules PWA 0.4.2 được kiểm thử bằng Firestore Emulator trước khi deploy vào project `chartforge-rsi`; workflow Pages không tự động thay đổi rules.
...
- PWA 0.4.2 tự làm sạch các field local-only do bản cũ để lại trong tài liệu settings trước khi retry, nên hàng đợi 403 hiện có tự đồng bộ lại mà không phải xóa dữ liệu. Trạng thái thị trường nằm trước nút tài khoản ở mép phải topbar; tiêu đề tab hiển thị giá live của symbol hiện tại từ REST và WebSocket.
