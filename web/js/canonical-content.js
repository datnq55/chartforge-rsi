// GENERATED from ../../content.js by scripts/build-pwa-canonical.mjs. Do not edit.
(() => {
  "use strict";
  ...
  function enablePan(canvas) {
    ...
    canvas.addEventListener("pointermove", (e) => {
      ...
      if (mode === "timeZoom") { const nextZoom = Math.max(20, Math.min(1000, Math.round(startZoom + (e.clientX - startX) / TIME_SCALE_DRAG_PX_PER_BAR))), nextStart = Math.round(anchorIndex - anchorRatio * (nextZoom - 1)); state.zoomBars = nextZoom; state.panBars = clampPan(state.closes.length - nextZoom - nextStart, nextZoom); const slider = shadow.querySelector(".zoom input"); slider.value = state.zoomBars; shadow.querySelector(".zoom output").value = state.zoomBars; }
